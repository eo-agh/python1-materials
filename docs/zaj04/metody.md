...
Zapoznaj się z nowym kodem w `python1course.zajecia04.fleet.ambulance`.

Metoda klasy - `get_instances_count` - odnosi się do samej klasy, a nie konkretnej instancji.

Metoda statyczna - `validate_id` - metoda pomocniczna, nie ma dostępu do obiektu klasy czy instacji, gdybyśmy odnosili się do atrybutu klasy `__instances_count` to nie moglibyśmy już mówić o metodzie statycznej.
