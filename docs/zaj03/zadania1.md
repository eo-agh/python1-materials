...
- Atrybut klasy `__instances_count` - współdzielony przez wszystkie instancje, zlicza utworzone karetki (patrz: [Atrybuty klasy vs instancji](klasy.md)),
- Użycie `isinstance()` w metodzie `__eq__` - sprawdza, czy drugi operand jest w ogóle instancją `Ambulance` przed porównaniem (patrz: [Introspekcja](introspekcja.md)),
- Właściwość `status` zdefiniowaną przez `@property` - setter waliduje dozwolone wartości i rzuca `ValueError` dla nieprawidłowych (patrz: [Metody - `@property`](metody.md)).
